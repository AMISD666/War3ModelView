...
class State {
    ...
    public str(length: number) {
        ...
        const bytes = this.uint.slice(this.pos, this.pos + stringLength);
        this.pos += length;

        // Try to decode as GBK first (common for Chinese WC3 models), then UTF-8, then Latin-1
        // Check if bytes contain high-bit characters that might be Chinese
        ...
        if (hasHighBit) {
            // Try GBK decoding for Chinese characters
            try {
        ...
        // ASCII-only strings can use simple conversion
