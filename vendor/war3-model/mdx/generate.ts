...
function generateExtent(obj: ObjWithExtent, stream: Stream): void {
    ...
    for (const key of ['MinimumExtent', 'MaximumExtent']) {
        // Fix: Use default [0,0,0] if undefined to prevent crash
        const val = obj[key] || new Float32Array(3);
        stream.float32Array(val);
    }
}
...
function generateMaterials(model: Model, stream: Stream): void {
    ...
    // console.log('[generateMaterials] Starting with', model.Materials.length, 'materials, Version:', model.Version);
    stream.keyword('MTLS');
    stream.int32(byteLengthMaterials(model) - 8);

    for (let mi = 0; mi < model.Materials.length; mi++) {
        const material = model.Materials[mi];
        const matSize = byteLengthMaterial(model, material);
        // console.log(`[generateMaterials] Material[${mi}]: size=${matSize}, Layers=${material.Layers?.length}`);

        stream.int32(matSize);
        ...
        for (let li = 0; li < material.Layers.length; li++) {
            const layer = material.Layers[li];
            const layerSize = byteLengthLayer(model, layer);
            stream.int32(layerSize);
...
function generateGeosets(model: Model, stream: Stream): void {
    ...
    for (const geoset of safeGeosets) {
        ...
        stream.uint8Array(geoset.VertexGroup instanceof Uint8Array ? geoset.VertexGroup : new Uint8Array(geoset.VertexGroup));
...
function generateParticleEmitters2(model: Model, stream: Stream): void {
    ...
    for (const emitter of model.ParticleEmitters2) {
        ...
        if (emitter.FrameFlags & ParticleEmitter2FramesFlags.Head &&
            ...
            // Write -1 for "Neither" to avoid defaulting to Head (0) upon reload in our parser
            stream.int32(-1);
        }
