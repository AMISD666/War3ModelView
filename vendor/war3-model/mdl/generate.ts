...
function generateModel(model: Model): string {
    return generateBlockStart('Model', model.Info.Name) +
        ...
        // Only output extent if not {0,0,0} - matches WC3/Green Apple behavior
        (isNotEmptyVec3(model.Info.MinimumExtent) ? generateFloatArrayProp('MinimumExtent', model.Info.MinimumExtent) : '') +
        (isNotEmptyVec3(model.Info.MaximumExtent) ? generateFloatArrayProp('MaximumExtent', model.Info.MaximumExtent) : '') +
...
function generateGeosetChunk(model: Model, geoset: Geoset): string {
    ...
    return generateBlockStart('Geoset') +
        ...
        generateSequenceGeosetAnimInfos(model, geoset) +
        (geoset.MaterialID !== null && geoset.MaterialID !== undefined ? generateIntProp('MaterialID', geoset.MaterialID) : '') +
        (geoset.SelectionGroup !== null && geoset.SelectionGroup !== undefined ? generateIntProp('SelectionGroup', geoset.SelectionGroup) : '') +
...
function generateSequenceGeosetAnimInfos(model: Model, geoset: Geoset): string {
    const sequenceCount = model.Sequences?.length || 0;
    if (!sequenceCount) {
        return generateGeosetAnimInfos(geoset.Anims);
    }

    const anims: GeosetAnimInfo[] = [];
    const existingAnims = geoset.Anims || [];

    for (let i = 0; i < sequenceCount; ++i) {
        anims.push(existingAnims[i] || {
            MinimumExtent: geoset.MinimumExtent,
            MaximumExtent: geoset.MaximumExtent,
            BoundsRadius: geoset.BoundsRadius
        });
    }

    return generateGeosetAnimInfos(anims);
}
...
function generateGeosetAnimColorProp(geosetAnim: GeosetAnim): string {
    ...
    if (color instanceof Float32Array) {
        if (isNotEmptyVec3(color, 1)) {
            return generateFloatArrayProp('Color', color, true);
        }
    } else {
        return generateAnimVectorProp('Color', color, null);
    }
...
/**
 * Generate node properties without DontInherit flags.
 * Used for ParticleEmitter2 since its Flags field uses different bit meanings
 * (ParticleEmitter2Flags like Unshaded) rather than NodeFlags.
 */
function generateNodePropsWithoutDontInherit(node: Node): string {
    return generateIntProp('ObjectId', node.ObjectId) +
        generateIntPropIfNotEmpty('Parent', node.Parent, null) +
        // Skip generateNodeDontInherit() - particle flags don't use NodeFlags low bits
        (node.Flags & NodeFlags.Billboarded ? generateBooleanProp('Billboarded') : '') +
        (node.Flags & NodeFlags.BillboardedLockX ? generateBooleanProp('BillboardedLockX') : '') +
        (node.Flags & NodeFlags.BillboardedLockY ? generateBooleanProp('BillboardedLockY') : '') +
        (node.Flags & NodeFlags.BillboardedLockZ ? generateBooleanProp('BillboardedLockZ') : '') +
...
function generateSegmentColor(colors: Float32Array[]): string {
    return generateBlockStart('SegmentColor', null, 1) +
        colors.map(color => generateFloatArrayProp('Color', color, null, 2)).join('') +
        generateTab() + '},\n';
}
...
function generateParticleEmitter2Chunk(particleEmitter2: ParticleEmitter2) {
    // NOTE: ParticleEmitter2 uses a different Flags structure than Node.
    // The low bits (DontInherit flags) should be masked out when checking node flags,
    // because ParticleEmitter2's Flags field is for particle-specific flags (Unshaded, etc.).
    // We generate node props manually, skipping DontInherit for particles.
    return generateBlockStart('ParticleEmitter2', particleEmitter2.Name) +
        generateNodePropsWithoutDontInherit(particleEmitter2) +
        ...
        generateIntPropIfNotEmpty('ReplaceableId', particleEmitter2.ReplaceableId, 0) +
        ...
        (generateParticleEmitter2FrameFlags(particleEmitter2.FrameFlags) ? generateBooleanProp(generateParticleEmitter2FrameFlags(particleEmitter2.FrameFlags)) : '') +
        generateBlockEnd();
}
