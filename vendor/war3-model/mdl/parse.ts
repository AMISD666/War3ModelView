...
function parseDontInheritFlags(state: State): NodeFlags {
    let flags = 0;
    ...
    while (state.char() !== '}') {
        ...
        if (val === 'Translation') {
            flags |= NodeFlags.DontInheritTranslation;
        } else if (val === 'Rotation') {
            flags |= NodeFlags.DontInheritRotation;
        } else if (val === 'Scaling') {
            flags |= NodeFlags.DontInheritScaling;
        } else {
            throwError(state, 'expected DontInherit flag');
        }
...
function parseGeosetPart(state: State, countPerObj: number, type: GeosetPartType, out?: Uint16Array, knownCount?: number) {
    let count: number;

    if (knownCount !== undefined) {
        count = knownCount;
    } else {
        const parsed = parseNumber(state);
        count = parsed !== null ? parsed : 0;
    }
    ...
    if (out) {
        ...
        if (type === GeosetPartType.FLOAT) {
            arr = new Float32Array(count * countPerObj);
        } else if (type === GeosetPartType.SHORT) {
            arr = new Uint16Array(count * countPerObj);
        } else {
            arr = new Uint8Array(count * countPerObj);
        }
    }
...
function parseNode(state: State, type: string, model: Model): Node {
    ...
    while (state.char() !== '}') {
        ...
        if (keyword === 'Translation' || keyword === 'Rotation' || keyword === 'Scaling' || keyword === 'Visibility') {
            ...
            node.Flags |= parseDontInheritFlags(state);
...
function parseParticleEmitter2(state: State, model: Model): void {
    ...
    while (state.char() !== '}') {
        ...
        if (!isStatic && (keyword === 'Speed' || keyword === 'Latitude' || keyword === 'Visibility' ||
            ...
            res.Flags |= parseDontInheritFlags(state);
