...
@vertex fn vs(
    ...
    sum += getBoneMatrix(in.skin[0]) * in.boneWeight[0];
    sum += getBoneMatrix(in.skin[1]) * in.boneWeight[1];
    sum += getBoneMatrix(in.skin[2]) * in.boneWeight[2];
    sum += getBoneMatrix(in.skin[3]) * in.boneWeight[3];
...
const gamma: f32 = 2.2;
const MAX_REFLECTION_LOD: f32 = 8.0;
...
@fragment fn fs(
    ...
    return vec4f(color, baseColor.a * fsUniforms.layerAlpha);
}
