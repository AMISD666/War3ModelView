...
const tailDirection = vec3.create();
const tailViewDirection = vec3.create();
...
const PARTICLE_TAIL_EPSILON = 0.000001;
...
function getParticleEmitter2FrameFlags(props: ParticleEmitter2): number {
    const explicitHead = (props as any)?.Head;
    const explicitTail = (props as any)?.Tail;
    if (typeof explicitHead === 'boolean' || typeof explicitTail === 'boolean') {
        return (explicitHead === true ? ParticleEmitter2FramesFlags.Head : 0) |
            (explicitTail === true ? ParticleEmitter2FramesFlags.Tail : 0);
    }

    const frameFlags = props?.FrameFlags;
    if (typeof frameFlags !== 'number' || !Number.isFinite(frameFlags)) {
        return ParticleEmitter2FramesFlags.Head;
    }
    if (frameFlags === 0) {
        return ParticleEmitter2FramesFlags.Head;
    }
    return frameFlags & (ParticleEmitter2FramesFlags.Head | ParticleEmitter2FramesFlags.Tail);
}
...
function scalarAnimSignature(value: any): string {
    if (value == null || typeof value !== 'object') {
        return String(value ?? '');
    }
    const keys = Array.isArray(value.Keys) ? value.Keys : [];
    return [
        value.LineType ?? value.InterpolationType ?? '',
        value.GlobalSeqId ?? '',
        keys.map((key: any) => {
            const vector = Array.isArray(key?.Vector) ? key.Vector : [];
            return `${key?.Frame ?? ''}:${vector.join(',')}`;
        }).join('|')
    ].join(';');
}
...
export class ParticlesController {
    ...
    private resetEmitterFrameBuffers(emitter: ParticleEmitterWrapper): void {
        if (this.gl) {
            if (emitter.headVertexBuffer) this.gl.deleteBuffer(emitter.headVertexBuffer);
            if (emitter.tailVertexBuffer) this.gl.deleteBuffer(emitter.tailVertexBuffer);
            if (emitter.headTexCoordBuffer) this.gl.deleteBuffer(emitter.headTexCoordBuffer);
            if (emitter.tailTexCoordBuffer) this.gl.deleteBuffer(emitter.tailTexCoordBuffer);
            if (emitter.colorBuffer) this.gl.deleteBuffer(emitter.colorBuffer);
            if (emitter.indexBuffer) this.gl.deleteBuffer(emitter.indexBuffer);
        }
    ...
    private updateParticleVertices(particle: Particle, index: number, emitter: ParticleEmitterWrapper,
        ...
        if (isModelSpace) {
            ...
            vec3.transformMat4(particleWorldPos, particle.pos, emitterMatrix);
            mat3.fromMat4(emitterRotationMat3, emitterMatrix);
            vec3.transformMat3(particleWorldSpeed, particle.speed, emitterRotationMat3);
        } else {
        ...
        if (emitter.type & ParticleEmitter2FramesFlags.Tail) {
            const tailLengthValue = Number(emitter.props.TailLength);
            const tailLength = Number.isFinite(tailLengthValue)
                ? tailLengthValue
                : 0;
            const speedLength = vec3.length(particleWorldSpeed);
            if (speedLength > PARTICLE_TAIL_EPSILON) {
                vec3.scale(tailPos, particleWorldSpeed, -tailLength);
                vec3.scale(tailDirection, particleWorldSpeed, (tailLength < 0 ? -1 : 1) / speedLength);
            } else {
                const emitterMatrix = this.rendererData.nodes[emitter.props.ObjectId].matrix;
                mat3.fromMat4(emitterRotationMat3, emitterMatrix);
                vec3.set(tailDirection, 0, 0, 1);
                vec3.transformMat3(tailDirection, tailDirection, emitterRotationMat3);
                if (vec3.length(tailDirection) <= PARTICLE_TAIL_EPSILON) {
                    vec3.set(tailDirection, 0, 0, 1);
                } else {
                    vec3.normalize(tailDirection, tailDirection);
                }

                if (tailLength < 0) {
                    vec3.negate(tailDirection, tailDirection);
                }
                vec3.scale(tailPos, tailDirection, -tailLength);
            }

            vec3.set(tailViewDirection, 0, 0, 1);
            vec3.transformQuat(tailViewDirection, tailViewDirection, this.rendererData.cameraQuat);

            vec3.cross(tailCross, tailViewDirection, tailDirection);
            if (vec3.length(tailCross) <= PARTICLE_TAIL_EPSILON) {
                vec3.set(tailFallbackAxis, 1, 0, 0);
                vec3.transformQuat(tailFallbackAxis, tailFallbackAxis, this.rendererData.cameraQuat);
                vec3.cross(tailCross, tailFallbackAxis, tailDirection);
            }
            if (vec3.length(tailCross) <= PARTICLE_TAIL_EPSILON) {
