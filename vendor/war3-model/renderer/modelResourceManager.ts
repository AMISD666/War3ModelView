    ...
    private initGPUBuffers(model: Model) {
        ...
        for (let i = 0; i < model.Geosets.length; ++i) {
            ...
            if (isHD && geoset.SkinWeights && geoset.Tangents) {
                ...
                new Uint8Array(buffers.groupBuffer[i].getMappedRange()).set(buffer);
    ...
    public addGeosetBuffers(model: Model, geosetIndex: number): void {
        ...
        if (this.device && this.gpuBuffers.has(model)) {
            ...
            if (isHD) {
                ...
                const groupData = new Uint8Array(geoset.VertexGroup.length * 4);
                ...
                new Uint8Array(buffers.groupBuffer[geosetIndex].getMappedRange()).set(groupData);
                buffers.groupBuffer[geosetIndex].unmap();
            }
