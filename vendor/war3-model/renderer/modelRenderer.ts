...
import type { ParticleQualityMode } from './particles';
import {
    applyWar3LayerBlendState,
...
export class ModelRenderer {
    ...
    public setParticleQualityMode(mode: ParticleQualityMode): void {
        this.modelInstance.particlesController.setParticleQualityMode(mode);
    }
    ...
    public renderInstances(instances: ModelInstance[], viewMatrix: mat4, pMatrix: mat4, {
        ...
        const renderGeoset = (i: number, renderPass: 'opaque' | 'transparent') => {
            ...
            if (this.isHD) {
                ...
                if (renderPass === 'opaque' && mode === FilterMode.None) hasRelevantLayers = true;
                else if (renderPass === 'transparent' && mode !== FilterMode.None) hasRelevantLayers = true;
            } else {
                for (const layer of material.Layers) {
                    const mode = layer.FilterMode || 0;
                    if (renderPass === 'opaque' && mode === FilterMode.None) { hasRelevantLayers = true; break; }
                    if (renderPass === 'transparent' && mode !== FilterMode.None) { hasRelevantLayers = true; break; }
                }
            }
            ...
            if (this.isHD) {
                ...
                if (renderPass === 'transparent') {
                    layerOrder.sort((a, b) => {
                        ...
                        return getWar3FilterDrawRank(modeA) - getWar3FilterDrawRank(modeB) || a - b;
                    });
                }
                ...
                for (const j of layerOrder) {
                    ...
                    if (renderPass === 'opaque' && mode !== FilterMode.None) continue;
                    if (renderPass === 'transparent' && mode === FilterMode.None) continue;
        ...
        if (this.isHD) {
            ...
            const bindModelShaderAttributes = () => {
                this.gl.useProgram(this.shaderProgram);
                this.gl.enableVertexAttribArray(this.shaderProgramLocations.vertexPositionAttribute);
                this.gl.enableVertexAttribArray(this.shaderProgramLocations.normalsAttribute);
                this.gl.enableVertexAttribArray(this.shaderProgramLocations.textureCoordAttribute);
            ...
            for (let i = 0; i < this.model.Geosets.length; ++i) {
                ...
                for (let j = 0; j < material.Layers.length; ++j) {
                    const layer = material.Layers[j];
                    const mode = layer.FilterMode || 0;
                    if (mode === FilterMode.None) continue;
            ...
            for (let instanceIndex = 0; instanceIndex < instances.length; ++instanceIndex) {
                ...
                const instanceMV = mat4.create();
                mat4.multiply(instanceMV, viewMatrix, instance.worldMatrix);
                particleMvMatrices[instanceIndex] = instanceMV;
                ...
                for (const item of instance.particlesController.getRenderItems(camPos)) {
                    transparentEntries.push({
                        kind: 'particle2',
                        instanceIndex,
                        emitterIndex: item.emitterIndex,
                        filterMode: mapParticleEmitter2FilterMode(item.filterMode),
    ...
    private setLayerProps(instance: ModelInstance, layer: Layer, textureID: number): boolean {
        ...
        this.gl.uniform1f(this.shaderProgramLocations.discardAlphaLevelUniform, getLayerDiscardAlphaLevel(layer.FilterMode || 0));
        applyWar3LayerBlendState(this.gl, layer.FilterMode || 0, layer.Shading || 0);
