...
import type{LiveTextureAdjustPayload,TextureReloadRequest,TextureReloadSchedulerState}from"./ViewerTextureUtils";import{GlobalTransformCommand}from"../../commands/GlobalTransformCommand";import{copyVertices,copyFaces,VertexCopyBuffer}from"../../utils/vertexOperations";import{UpdateKeyframeCommand,KeyframeChange}from"../../commands/UpdateKeyframeCommand";import{MissingTextureWarning}from"../MissingTextureWarning";import{GeosetSeparateDialog}from"../modals/GeosetSeparateDialog";import{LayerConfig,layerConfigToMaterialLayer}from"../modals/MaterialLayerOptions";import{NodeType}from"../../types/node";import{openNodeEditor}from"../../utils/nodeEditorOpen";import{nodeTypeToEditorKind}from"../../types/nodeEditorRpc";import{getEffectiveBindings,registerShortcutHandler}from"../../shortcuts/manager";import{markStandalonePerf}from"../../utils/standalonePerf";import{invokeReadMpqFile}from"../../utils/mpqPerf";import{markNodeManagerListScrollFromTree,markNodeManagerListScrollFromViewer}from"../../utils/nodeManagerListScrollBridge";import{collectSelectedGeosetIndices}from"../editors/uvSelectionSync";import{createSelectionRect,pointInRect,segmentIntersectsRect,triangleIntersectsRect}from"../editors/uvSelectionUtils";let globalRenderLoopId=0;const Viewer=forwardRef((props:ViewerProps,ref:React.Ref<ViewerRef>)=>{const{modelPath,animationIndex,teamColor,showGrid,showNodes,showSkeleton,showCollisionShapes,showCameras,showLights,showAttachments,showWireframe,showWireframeOverlay=false,isPlaying,onTogglePlay,onToggleLooping,onToggleWireframe,onModelLoaded,onModelFirstFrameReady,onModelLoadError,backgroundColor,showFPS,playbackSpeed,viewPreset,modelData,onSetViewPreset,onAddCameraFromView}=props;const[parseWorker]=useState(()=>new ModelWorker());const[textureWorkers]=useState<WorkerLike[]>(()=>{const count=getTextureDecodeWorkerCount();return Array.from({length:count},()=>new ModelWorker()as unknown as WorkerLike);});const[loading,setLoading]=useState(false);const[loadingStatus,setLoadingStatus]=useState("");const canvasRef=useRef<HTMLCanvasElement>(null);const[renderer,setRenderer]=useState<War3ModelRenderer|null>(null);const[fps,setFps]=useState<number>(0);const gridRenderer=useRef(new GridRenderer());const debugRenderer=useRef(new DebugRenderer());const gizmoRenderer=useRef(new GizmoRenderer());const axisIndicator=useRef(new AxisIndicator());const rendererRef=useRef<War3ModelRenderer|null>(null);const cameraRef=useRef<SimpleOrbitCamera|null>(null);const appMainMode=useSelectionStore((state)=>state.mainMode);const animationSubMode=useSelectionStore((state)=>state.animationSubMode);const rendererReloadTrigger=useModelStore((state)=>state.rendererReloadTrigger);const cachedRenderer=useModelStore((state)=>state.cachedRenderer);const mpqLoaded=useRendererStore((state)=>state.mpqLoaded);const nodeRenderMode=useRendererStore((state)=>state.nodeRenderMode);const showHealthBar=useRendererStore((state)=>state.showHealthBar);const missingTextures=useRendererStore((state)=>state.missingTextures);const glRef=useRef<WebGL2RenderingContext|WebGLRenderingContext|null>(null);const needsRendererUpdateRef=useRef(false);const bindingParticlePoseSignatureRef=useRef("");const textureReloadSchedulerRef=useRef<TextureReloadSchedulerState>({timer:null,running:false,queued:null,version:0,});const animationFrameId=useRef<number|null>(null);const shouldRunRenderLoop=useRef<boolean>(true);const lastRenderErrorReportTimeRef=useRef<number>(0);const framePerfRef=useRef<ViewerFramePerfAggregate>(createViewerFramePerfAggregate());const lastFpsTime=useRef<number>(performance.now());const lastFrameTime=useRef<number>(performance.now());const frameCount=useRef<number>(0);const renderRef=useRef<((time:number,scheduleNext?:boolean)=>void)|null>(null);const pMatrixRef=useRef(mat4.create());const mvMatrixRef=useRef(mat4.create());const cameraPosRef=useRef(vec3.create());const cameraUpRef=useRef(vec3.fromValues(0,0,1));const cameraQuatRef=useRef(quat.create());const showModelInfo=useUIStore((state)=>state.showModelInfo);const isLooping=useModelStore((state)=>state.isLooping);const setLooping=useModelStore((state)=>state.setLooping);const[texturePreview,setTexturePreview]=useState<{url:string;width:number;height:number;path:string;}|null>(null);const backgroundTextureResolveRunningRef=useRef(false);const attemptedMissingTexturePathsRef=useRef<Set<string>>(new Set());const flushFramePerfSummary=useCallback((reason:string,force=false)=>{const bucket=framePerfRef.current;if(bucket.samples===0)return;if(!force&&bucket.samples<90)return;markStandalonePerf("viewer_frame_profile",{reason,samples:bucket.samples,avgTotalMs:roundPerfValue(bucket.totalMs/bucket.samples),maxTotalMs:roundPerfValue(bucket.maxTotalMs),slowFrameCount:bucket.slowFrameCount,avgClearMs:roundPerfValue(bucket.clearMs/bucket.samples),avgCameraMs:roundPerfValue(bucket.cameraMs/bucket.samples),avgStateMs:roundPerfValue(bucket.stateMs/bucket.samples),avgUpdateMs:roundPerfValue(bucket.updateMs/bucket.samples),avgSceneMs:roundPerfValue(bucket.sceneMs/bucket.samples),avgOverlayMs:roundPerfValue(bucket.overlayMs/bucket.samples),isPlaying:isPlayingRef.current,modelPath:modelPath||"",});framePerfRef.current=createViewerFramePerfAggregate();},[modelPath],);const recordFramePerfSample=useCallback((sample:ViewerFramePerfSample,detail?:Record<string,unknown>)=>{const bucket=framePerfRef.current;bucket.samples+=1;bucket.totalMs+=sample.totalMs;bucket.maxTotalMs=Math.max(bucket.maxTotalMs,sample.totalMs);bucket.clearMs+=sample.clearMs;bucket.cameraMs+=sample.cameraMs;bucket.stateMs+=sample.stateMs;bucket.updateMs+=sample.updateMs;bucket.sceneMs+=sample.sceneMs;bucket.overlayMs+=sample.overlayMs;if(sample.totalMs>=16.7){bucket.slowFrameCount+=1;};const now=performance.now();if(sample.totalMs>=33&&now-bucket.lastSlowEmitMs>=1000){bucket.lastSlowEmitMs=now;markStandalonePerf("viewer_slow_frame",{totalMs:roundPerfValue(sample.totalMs),clearMs:roundPerfValue(sample.clearMs),cameraMs:roundPerfValue(sample.cameraMs),stateMs:roundPerfValue(sample.stateMs),updateMs:roundPerfValue(sample.updateMs),sceneMs:roundPerfValue(sample.sceneMs),overlayMs:roundPerfValue(sample.overlayMs),...detail,});};if(bucket.samples>=90){flushFramePerfSummary("periodic");}},[flushFramePerfSummary],);useEffect(()=>{return()=>{parseWorker.terminate();textureWorkers.forEach((worker)=>worker.terminate?.());};},[parseWorker,textureWorkers]);const reportModelLoadError=(error:unknown,path:string|null)=>{if(onModelLoadError){onModelLoadError(error,path);return;}const message=error instanceof Error?error.message:String(error);appMessage.error({content:`模型加载失败：${message}`,duration:6000});};const formatCameraValue=(value:number):string=>{if(!Number.isFinite(value))return"0";const formatted=value.toFixed(2);return formatted.replace(/.?0+$/,"");};const getCameraVector=(prop:any,directProp?:any):number[]=>{const isArrayLike=(v:any)=>Array.isArray(v)||v instanceof Float32Array||ArrayBuffer.isView(v);const toArray=(v:any)=>(v instanceof Float32Array?Array.from(v):v);if(directProp&&isArrayLike(directProp))return toArray(directProp);if(isArrayLike(prop))return toArray(prop);if(prop&&prop.Keys&&prop.Keys.length>0){const v=prop.Keys[0].Vector;return v?toArray(v):[0,0,0];};return[0,0,0];};const getAvailableCameras=():any[]=>{const{modelData,nodes}=useModelStore.getState();const modelCameras=Array.isArray((modelData as any)?.Cameras)?(modelData as any).Cameras.filter((cam:any)=>cam):[];if(modelCameras.length>0)return modelCameras;return Array.isArray(nodes)?nodes.filter((n:any)=>n&&n.type==="Camera"):[];};const getSelectedCamera=(cameraIndex=selectedCameraIndex):any|null=>{const cameraList=getAvailableCameras();if(cameraIndex<0||cameraIndex>=cameraList.length)return null;return cameraList[cameraIndex]??null;};const clearActiveModelCameraView=()=>{inCameraView.current=false;setActiveModelCameraView(null);};const roundVertexCoord=(value:number):number=>Math.round(value*10000)/10000;const getVertexPositionKey=(vertices:ArrayLike<number>,vertexIndex:number):string=>{const base=vertexIndex*3;return`${roundVertexCoord(Number(vertices[base]??0))},${roundVertexCoord(Number(vertices[base+1]??0))},${roundVertexCoord(Number(vertices[base+2]??0))}`;
    ...
    if (state.mainMode === "view") return "查看模式";
    if (state.mainMode === "animation") {
      return state.animationSubMode === "keyframe" ? "关键帧模式" : "绑定模式";
    }
    if (state.mainMode === "uv") {
      const uvModeNames: Record<typeof state.uvSelectionMode, string> = {
        vertex: "UV 顶点模式",
        edge: "UV 边模式",
        face: "UV 面模式",
        group: "UV 成组模式",
      ...
      return uvModeNames[state.uvSelectionMode];
    }
    const geometryModeNames: Record<typeof state.geometrySubMode, string> = {
      vertex: "顶点模式",
      face: "面模式",
      group: "成组模式",
    };
    return geometryModeNames[state.geometrySubMode];
  });

  const getExpandedFaceVertexSelection = (selectedFaceIds: Array<{ geosetIndex: number; index: number }>): Array<{ geosetIndex: number; index: number }> => {
    ...
    selectedFaceIds.forEach((sel) => {
      ...
      const indices = [Number(geoset.Faces[fIndex]), Number(geoset.Faces[fIndex + 1]), Number(geoset.Faces[fIndex + 2])].filter((value) => Number.isFinite(value));
  ...
  const isCurrentGeosetVisible = (geosetIndex: number): boolean => {
    const { hiddenGeosetIds, forceShowAllGeosets } = useModelStore.getState();
    return isGeosetVisible(geosetIndex, hiddenGeosetIds, forceShowAllGeosets);
  };
  ...
  const getConnectedGeometryGroup = (
    ...
    for (let faceIndex = 0; faceIndex < faceCount; faceIndex++) {
      ...
      const vertexIndices = [Number(geoset.Faces[fIndex]), Number(geoset.Faces[fIndex + 1]), Number(geoset.Faces[fIndex + 2])];
  ...
  const queueUvSelectionSyncFromStore = () => {
    const selectionState = useSelectionStore.getState();
    if (selectionState.mainMode !== "uv") {
      return;
    }

    const geosetIndices = collectSelectedGeosetIndices(selectionState.selectedVertexIds, selectionState.selectedFaceIds);
    useUvEditorStore.getState().queueViewerSelectionSync(geosetIndices);
  };
  ...
  const pickHealthBar = useCallback(
    (cameraPos: vec3, rayDir: vec3) => {
      ...
      return Math.abs(localX) <= billboard.halfWidth + paddingX && Math.abs(localY) <= billboard.halfThickness + paddingY;
    },
  ...
  useEffect(() => {
    if (cachedRenderer && cachedRenderer.model) {
      const modelPathStr = modelPath || (cachedRenderer as any).__modelPath || cachedRenderer.model.path || "unknown";
      (cachedRenderer as any).__modelPath = modelPathStr;
      ...
      // Tab snapshots are authoritative during hot-swap. Re-writing the store from
      // cachedRenderer.model can copy stale renderer-side GeosetAnims into this tab.
  ...
  const particleQualityModeRef = useRef(DEFAULT_PARTICLE_QUALITY_MODE);
  ...
  const getShowVerticesForCurrentContext = () => {
    ...
    if (sel.mainMode === "animation") {
      return sel.animationSubMode === "binding" ? (state.showVerticesInAnimationBinding ?? true) : (state.showVerticesInAnimationKeyframe ?? false);
    }
  ...
  const setGlobalRenderer = useRendererStore((state) => state.setRenderer);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const areAdjustmentsEqual = (left: TextureAdjustments, right: TextureAdjustments) => left.hue === right.hue && left.brightness === right.brightness && left.saturation === right.saturation && left.opacity === right.opacity;
    ...
    const readTextureSource = async (modelPathValue: string, imagePath: string): Promise<ImageData | null> => {
      ...
      if (modelPathValue && !modelPathValue.startsWith("dropped:")) {
        ...
        for (const candidate of candidates) {
          const buffer = await desktopGateway.readFile(candidate).catch(() => null);
          if (buffer) {
            const imageData = decodeTextureData(toTightArrayBuffer(buffer), imagePath);
    ...
    const ensureSourceImageData = async (modelPathValue: string, imagePath: string, assetRevision?: number): Promise<ImageData | null> => {
      const sourceKey = getLiveTextureSourceKey(modelPathValue, imagePath, assetRevision);
    ...
    const processQueuedAdjust = async () => {
      ...
      while (queuedAdjust) {
        ...
        const sourceKey = getLiveTextureSourceKey(current.modelPath, current.imagePath, current.assetRevision);
        const sourceImageData = await ensureSourceImageData(current.modelPath, current.imagePath, current.assetRevision);
        if (!sourceImageData) {
          continue;
        }
    ...
    const unlistenPrepare = windowGateway.listen("IPC_LIVE_TEXTURE_PREPARE", (event) => {
      const payload: any = (event as any).payload;
      if (!payload?.modelPath || !payload?.imagePath) {
        return;
      }
      void ensureSourceImageData(String(payload.modelPath), payload.imagePath, Number(payload?.assetRevision ?? 0));
    });

    const unlistenAdjust = windowGateway.listen("IPC_LIVE_TEXTURE_ADJUST", (event) => {
      const payload: any = (event as any).payload;
      ...
      queuedAdjust = {
        ...
        assetRevision: Number(payload?.assetRevision ?? 0),
  ...
  const applySelectedCameraView = useCallback(
  ...
  const applyViewPreset = useCallback(
    (preset: string, options?: { syncExternal?: boolean }) => {
      ...
      getCamera: () => {
        ...
        if (cameraRef.current) {
          return {
            ...
            target: [cameraRef.current.target[0], cameraRef.current.target[1], cameraRef.current.target[2]] as [number, number, number],
          };
        }
        ...
        return {
          ...
          theta: targetCamera.current.theta,
          phi: targetCamera.current.phi,
          target: [targetCamera.current.target[0], targetCamera.current.target[1], targetCamera.current.target[2]] as [number, number, number],
        };
      },
  ...
  const buildHudText = (mode: "translate" | "rotate" | "scale") => {
    ...
    if (mode === "translate") {
      return `移动Δ:(${formatHudVec3(hud.translation,3)})`;
    }
    if (mode === "rotate") {
      return `旋转Δ:(${formatHudVec3(hud.rotation,1)})°`;
    }
    return `缩放Δ:(${formatHudVec3(hud.scale,3)})`;
  };
    ...
    const loadTexture = async (path: string, id: number) => {
      try {
        ...
        if (mpqData && mpqData.length > 0) {
          ...
          const idata = decodeWar3BlpMipToImageData(toTightArrayBuffer(mpqData), 0);
  ...
  useEffect(() => {
    ...
    const loadedPaths = new Set<string>([...Object.keys(rendererData?.textures || {}), ...Object.keys(rendererData?.gpuTextures || {})]);

    const unresolved = pending.filter((path) => !loadedPaths.has(path) && !attemptedMissingTexturePathsRef.current.has(path));
    ...
    const run = async () => {
      ...
      try {
        const textureResults = await loadAllTextures(sourceModel, renderer, activePath, textureWorkers, undefined, {
          yieldUploads: true,
          targetPaths: unresolved,
          workerDecodeMinTextures: 10,
          workerDecodeMinBytes: 8 * 1024 * 1024,
        });
        const resolved = new Set(textureResults.filter((result) => result.loaded).map((result) => result.path));
        if (resolved.size > 0) {
  ...
  const sanitizeModelGeosetsForReload = (model: any, fallbackGeosets?: any[] | null) => {
    ...
    const fallbackMatches = Array.isArray(fallbackGeosets) && fallbackGeosets.length === model.Geosets.length;
    ...
    const sanitizedGeosets = model.Geosets.flatMap((geoset: any, index: number) => {
      ...
      const canRestoreFromFallback = !!fallbackGeoset && hasGeometryBufferData(fallbackGeoset.Vertices) && hasGeometryBufferData(fallbackGeoset.Faces);
        ...
        return [
          {
            ...
            MatrixIndices: hasGeometryBufferData(geoset.MatrixIndices) ? geoset.MatrixIndices : cloneGeometryBuffer(fallbackGeoset.MatrixIndices),
      ...
      skippedCount += 1;
      console.error(`[Viewer]Skipping geoset ${index}during full reload because Vertices/Faces are missing and no fallback geometry is available`);
      return [];
    });
  ...
  const stableStoreNodes = useMemo(() => (Array.isArray(storeNodes) ? storeNodes.filter((n: any) => n && typeof n.ObjectId === "number") : []), [storeNodes]);
  ...
  useEffect(() => {
    if (rendererRef.current && rendererRef.current.model) {
      ...
      if (!currentStore.modelData || currentStore.modelPath !== modelPath) {
        return;
      }
      const effectiveNodes = Array.isArray(storeNodes) && storeNodes.length > 0
        ? storeNodes
        : extractNodesFromModel(currentStore.modelData);
      if (effectiveNodes.length === 0 && Array.isArray(rendererRef.current.model.Nodes) && rendererRef.current.model.Nodes.length > 0) {
        return;
      }
      rendererSyncService.syncNodeProjection({
        renderer: rendererRef.current as any,
        nodes: effectiveNodes as any,
        documentRevision: currentStore.documentRevision,
        previewRevision: currentStore.previewRevision,
      });
  ...
  useEffect(() => {
    if (rendererRef.current && (rendererRef.current as any).modelInstance && structureUpdateTrigger > 0) {
      ...
      rendererSyncService.syncNodeStructure({
        renderer: rendererRef.current as any,
  ...
  useEffect(() => {
    if (rendererRef.current && (rendererRef.current as any).modelInstance && materialUpdateTrigger > 0) {
      ...
      const syncResult = rendererSyncService.syncMaterialProjection({
        ...
        documentRevision: modelState.documentRevision,
        previewRevision: modelState.previewRevision,
        prepareMaterialsForRenderer: (materials, texturesLength) =>
          cloneMaterialsWithReferenceBlendCompat(sanitizeMaterialsForRenderer(materials as any[] | undefined, texturesLength)),
      });

      if (syncResult.applied) {
        needsRendererUpdateRef.current = true;
      }
      if (syncResult.plan === "fullReload") {
  ...
  const activeModelLoadTokenRef = useRef(0);
  ...
  const handleBoxSelection = (startX: number, startY: number, endX: number, endY: number, isShift: boolean, isCtrl: boolean) => {
    ...
    const { mainMode, animationSubMode, geometrySubMode, uvSelectionMode, addVertexSelection, addFaceSelection, removeVertexSelection, removeFaceSelection, selectVertices, selectFaces, selectNodes } = useSelectionStore.getState();
    if (mainMode !== "geometry" && mainMode !== "animation" && mainMode !== "uv") return;
    ...
    const selectionRect = createSelectionRect({ x: boxLeft, y: boxTop }, { x: boxRight, y: boxBottom }, 0);
    ...
    const toCanvasPoint = (screenPos: vec3) => ({ x: screenPos[0], y: screenPos[1] });
    const isScreenPosInBox = (screenPos: vec3 | null) => screenPos ? pointInRect(toCanvasPoint(screenPos), selectionRect) : false;
    ...
    if (mainMode === "animation" && animationSubMode !== "binding") {
      ...
      rendererRef.current.rendererData.nodes.forEach((nodeWrapper: any) => {
        ...
        if (screenPos) {
          if (screenPos[0] >= boxLeft && screenPos[0] <= boxRight && screenPos[1] >= boxTop && screenPos[1] <= boxBottom) {
      ...
      const hitVertices: { geosetIndex: number; index: number }[] = [];
      const hitFaces: { geosetIndex: number; index: number }[] = [];
      const hitVertexKeys = new Set<string>();
      const hitFaceKeys = new Set<string>();
      const { hiddenGeosetIds, forceShowAllGeosets } = useModelStore.getState();

      const pushVertex = (geosetIndex: number, index: number) => {
        const key = `${geosetIndex}-${index}`;
        if (hitVertexKeys.has(key)) {
          return;
        }
        hitVertexKeys.add(key);
        hitVertices.push({ geosetIndex, index });
      };

      const pushFace = (geosetIndex: number, index: number) => {
        const key = `${geosetIndex}-${index}`;
        if (hitFaceKeys.has(key)) {
          return;
        }
        hitFaceKeys.add(key);
        hitFaces.push({ geosetIndex, index });
      };

      for (let geosetIndex = 0; geosetIndex < rendererRef.current.model.Geosets.length; geosetIndex++) {
        if (!isGeosetVisible(geosetIndex, hiddenGeosetIds, forceShowAllGeosets)) {
          continue;
        }
        ...
        const projectVertex = (vertexIndex: number) => {
          ...
          return project(vec3.fromValues(vertices[vertexOffset], vertices[vertexOffset + 1], vertices[vertexOffset + 2]));
        };

        if (uvSelectionMode === "vertex") {
          for (let vertexIndex = 0; vertexIndex < vertexCount; vertexIndex++) {
            if (isScreenPosInBox(projectVertex(vertexIndex))) {
              pushVertex(geosetIndex, vertexIndex);
            }
          }
          continue;
        ...
        if (uvSelectionMode === "edge") {
          ...
          for (let faceOffset = 0; faceOffset < faces.length; faceOffset += 3) {
            const i0 = Number(faces[faceOffset]);
            const i1 = Number(faces[faceOffset + 1]);
            const i2 = Number(faces[faceOffset + 2]);
            const p0 = projectVertex(i0);
            const p1 = projectVertex(i1);
            const p2 = projectVertex(i2);
            if (!p0 || !p1 || !p2) {
              continue;
            }

            if (segmentIntersectsRect(toCanvasPoint(p0), toCanvasPoint(p1), selectionRect)) {
              pushVertex(geosetIndex, i0);
              pushVertex(geosetIndex, i1);
            }
            if (segmentIntersectsRect(toCanvasPoint(p1), toCanvasPoint(p2), selectionRect)) {
              pushVertex(geosetIndex, i1);
              pushVertex(geosetIndex, i2);
            }
            if (segmentIntersectsRect(toCanvasPoint(p2), toCanvasPoint(p0), selectionRect)) {
              pushVertex(geosetIndex, i2);
              pushVertex(geosetIndex, i0);
            }
          }
        ...
        if (uvSelectionMode === "face") {
          ...
          for (let faceOffset = 0; faceOffset < faces.length; faceOffset += 3) {
            ...
            if (triangleIntersectsRect(toCanvasPoint(p0), toCanvasPoint(p1), toCanvasPoint(p2), selectionRect)) {
              pushFace(geosetIndex, faceIndex);
              pushVertex(geosetIndex, i0);
              pushVertex(geosetIndex, i1);
              pushVertex(geosetIndex, i2);
            }
        ...
        if (uvSelectionMode === "block" && faces) {
          ...
          if (seedFaceIndices.length === 0) {
            continue;
          }

          const blockSelection = getConnectedGeometryGroup(geosetIndex, seedFaceIndices, []);
          blockSelection.vertices.forEach((selection) => pushVertex(selection.geosetIndex, selection.index));
          blockSelection.faces.forEach((selection) => pushFace(selection.geosetIndex, selection.index));
        }
      }

      if (uvSelectionMode === "vertex" || uvSelectionMode === "edge") {
        if (isShift) {
          removeVertexSelection(hitVertices);
        } else if (isCtrl) {
          addVertexSelection(hitVertices);
        } else {
          selectVertices(hitVertices);
        }
        selectFaces([]);
        queueUvSelectionSyncFromStore();
        return;
      }

      if (isShift) {
        removeVertexSelection(hitVertices);
        removeFaceSelection(hitFaces);
      } else if (isCtrl) {
        addVertexSelection(hitVertices);
        addFaceSelection(hitFaces);
      } else {
        selectVertices(hitVertices);
        selectFaces(hitFaces);
      }
      queueUvSelectionSyncFromStore();
      return;
      ...
      const newFaceSelection: { geosetIndex: number; index: number }[] = [];
      ...
      for (let i = 0; i < rendererRef.current.model.Geosets.length; i++) {
        ...
        for (let j = 0; j < vertices.length; j += 3) {
          const screenPos = projectVertex(j / 3);
        ...
        if (geometrySubMode === "group" && faces) {
          for (let j = 0; j < faces.length; j += 3) {
            const s0 = projectVertex(Number(faces[j]));
            const s1 = projectVertex(Number(faces[j + 1]));
            const s2 = projectVertex(Number(faces[j + 2]));
            if (!s0 || !s1 || !s2) {
              continue;
            }

            if (triangleIntersectsRect(toCanvasPoint(s0), toCanvasPoint(s1), toCanvasPoint(s2), selectionRect)) {
              newFaceSelection.push({ geosetIndex: i, index: j / 3 });
            }
          }
        }
      }
      ...
      if (geometrySubMode === "group") {
        ...
        const vertexSeedsByGeoset = new Map<number, number[]>();
        const faceSeedsByGeoset = new Map<number, number[]>();

        newSelection.forEach((sel) => {
          const seeds = vertexSeedsByGeoset.get(sel.geosetIndex);
          if (seeds) {
            seeds.push(sel.index);
          } else {
            vertexSeedsByGeoset.set(sel.geosetIndex, [sel.index]);
          }
        });

        newFaceSelection.forEach((sel) => {
          const seeds = faceSeedsByGeoset.get(sel.geosetIndex);
          if (seeds) {
            seeds.push(sel.index);
          } else {
            faceSeedsByGeoset.set(sel.geosetIndex, [sel.index]);
          }
        });

        const seedGeosetIndices = new Set<number>([...vertexSeedsByGeoset.keys(), ...faceSeedsByGeoset.keys()]);
        seedGeosetIndices.forEach((geosetIndex) => {
          const groupSelection = getConnectedGeometryGroup(geosetIndex, faceSeedsByGeoset.get(geosetIndex) ?? [], vertexSeedsByGeoset.get(geosetIndex) ?? []);
      ...
      for (let i = 0; i < rendererRef.current.model.Geosets.length; i++) {
        ...
        for (let j = 0; j < faces.length; j += 3) {
          ...
          if (s0 && s1 && s2 && triangleIntersectsRect(toCanvasPoint(s0), toCanvasPoint(s1), toCanvasPoint(s2), selectionRect)) {
            newSelection.push({ geosetIndex: i, index: j / 3 });
          }
        }
      }
  ...
  const handleSelectionClick = (clientX: number, clientY: number, isShift: boolean, isCtrl: boolean, isAlt: boolean = false) => {
    ...
    const { mainMode, animationSubMode, geometrySubMode, uvSelectionMode, selectVertex, selectVertices, selectFace, selectFaces, addVertexSelection, addFaceSelection, removeVertexSelection, removeFaceSelection, clearAllSelections, selectNode, clearNodeSelection, setPickedGeosetIndex } = useSelectionStore.getState();
    ...
    // Priority: Node > Vertex (if clicked on both). View mode can also pick nodes
    // so the node manager can jump to the clicked item without switching modes.
    if (mainMode === "animation" || mainMode === "view") {
      ...
      if (rendererRef.current.rendererData && rendererRef.current.rendererData.nodes) {
        filterVisibleRendererNodes(rendererRef.current.rendererData.nodes, null, useRendererStore.getState().nodeTypeVisibility).forEach((nodeWrapper: any) => {
          const pivot = getOrCreateNodePivot(nodeWrapper);
      ...
      if (closestNodeId !== -1) {
        ...
        if (isPickingParent && selectedNodeIds.length === 1) {
          ...
          if (closestNodeId !== selectedNodeId) {
      ...
      if (mainMode === "view" || animationSubMode !== "binding") {
        return;
      }
    }

    if (mainMode !== "geometry" && mainMode !== "uv" && !(mainMode === "animation" && animationSubMode === "binding")) {
      if (!isShift && !isCtrl) {
        setHealthBarSelection(false);
      }
      return;
    ...
    const isUvMode = mainMode === "uv";
    const effectiveSubMode = isUvMode
      ? (uvSelectionMode === "face" || uvSelectionMode === "group" || uvSelectionMode === "block" ? "face" : "vertex")
      : geometrySubMode === "group" ? "face" : isBindingVertexMode ? "vertex" : geometrySubMode;
    ...
    if (result) {
      ...
      if (isUvMode) {
        ...
        if (effectiveSubMode === "vertex") {
          ...
          const applyUvSelection = (
            vertexSelection: Array<{ geosetIndex: number; index: number }>,
            faceSelection: Array<{ geosetIndex: number; index: number }>
          ) => {
            if (isShift) {
              removeVertexSelection(vertexSelection);
              removeFaceSelection(faceSelection);
            } else if (isCtrl) {
              addVertexSelection(vertexSelection);
              addFaceSelection(faceSelection);
            } else {
          ...
          if (uvSelectionMode === "face") {
            ...
            applyUvSelection(
              [
                { geosetIndex: sel.geosetIndex, index: Number(faces[faceOffset]) },
                { geosetIndex: sel.geosetIndex, index: Number(faces[faceOffset + 1]) },
                { geosetIndex: sel.geosetIndex, index: Number(faces[faceOffset + 2]) }
              ],
              [sel]
            );
          } else if (uvSelectionMode === "group") {
            const vertexSelection: Array<{ geosetIndex: number; index: number }> = [];
            const faceSelection: Array<{ geosetIndex: number; index: number }> = [];
            const vertexCount = Math.floor((geoset?.Vertices?.length ?? 0) / 3);
            const faceCount = Math.floor(faces.length / 3);

            for (let vertexIndex = 0; vertexIndex < vertexCount; vertexIndex++) {
              vertexSelection.push({ geosetIndex: sel.geosetIndex, index: vertexIndex });
            }
            for (let faceIndex = 0; faceIndex < faceCount; faceIndex++) {
              faceSelection.push({ geosetIndex: sel.geosetIndex, index: faceIndex });
            }

            applyUvSelection(vertexSelection, faceSelection);
          } else if (uvSelectionMode === "block") {
            const blockSelection = getConnectedGeometryGroup(sel.geosetIndex, [sel.index], []);
            applyUvSelection(blockSelection.vertices, blockSelection.faces);
          }
        }

        queueUvSelectionSyncFromStore();
      } else if (geometrySubMode === "group") {
        const sel = result as { geosetIndex: number; index: number };
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(() => {
      ...
      if (width > 0 && height > 0) {
        ...
        if (nodeNameOverlayRef.current) Object.assign(nodeNameOverlayRef.current, { width, height });
    ...
    const handleNativeWheel = (e: WheelEvent) => {
      if (e.ctrlKey || e.metaKey) {
        e.preventDefault();
        zoomNodeSizeFromWheel(e.deltaY);
      }
    };
  ...
  const initializeRendererBackend = (canvas: HTMLCanvasElement, newRenderer: War3ModelRenderer): void => {
    const gl = (canvas.getContext("webgl2", WEBGL_CONTEXT_ATTRIBUTES) as WebGL2RenderingContext | null) || (canvas.getContext("webgl", WEBGL_CONTEXT_ATTRIBUTES) as WebGLRenderingContext | null);
  ...
  const waitForIdleSlice = (timeoutMs = 150) =>
    new Promise<void>((resolve) => {
  ...
  const loadModel = async (path: string, inMemoryData?: any) => {
    lastFrameTime.current = performance.now();
    const loadToken = ++activeModelLoadTokenRef.current;
    const loadPerfStart = performance.now();
    const modelLoadId = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    const markLoadStage = (stage: string, detail: Record<string, unknown> = {}) => {
      markStandalonePerf("model_load_stage", {
        loadId: modelLoadId,
        stage,
        modelPath: path,
        elapsedMs: roundPerfValue(performance.now() - loadPerfStart),
        ...detail,
      });
    };
    const isCurrentLoad = () => activeModelLoadTokenRef.current === loadToken && lastLoadedModelPath.current === path;
    ...
    const canvas = canvasRef.current;
    if (!canvas) {
      const error = new Error("Viewer canvas is not ready");reportModelLoadError(error, path);setLoading(false);setLoadingStatus("");return;
    }
    if (canvas) {
    ...
    try {
      ...
      markLoadStage("start", { inMemory: !!inMemoryData });
      canvas.focus();

      const readPathBytes = async (assetPath: string): Promise<Uint8Array> => {
        try {
          return await desktopGateway.readFile(assetPath);
        } catch {
      ...
      // Texture preview mode (.blp/.tga/browser image)
      if (isTexturePreviewPath(path)) {
        const bytes = await readPathBytes(path);
        const imageData = await decodeTextureDataAsync(toTightArrayBuffer(bytes), path, { preferBlpBaseMip: true });
        if (!imageData) {
      ...
      if (inMemoryData) {
        model = inMemoryData;
        markLoadStage("parse_skipped_in_memory");
      } else {
        setLoadingStatus("正在解析模型...");
        const readStart = performance.now();
        const buffer = await readPathBytes(path);
        markLoadStage("read_model_bytes", {
          bytes: buffer.byteLength,
          stageMs: roundPerfValue(performance.now() - readStart),
        });
      ...
      markLoadStage("parse_complete", {
        stageMs: roundPerfValue(performance.now() - parseStart),
      });
      ...
      const textureContextStart = performance.now();
      ...
      const batchPayloadPromise =
        textureLoadContext.effectiveTexturePaths.length > 0
          ? desktopGateway.invoke<Uint8Array>("load_textures_batch_bin", {
              modelPath: path,
              texturePaths: textureLoadContext.effectiveTexturePaths,
            })
          : undefined;
      markLoadStage("texture_paths_prepared", {
        textureCount: textureLoadContext.effectiveTexturePaths.length,
        stageMs: roundPerfValue(performance.now() - textureContextStart),
      });
      ...
      markLoadStage("particle_validation_complete", {
        stageMs: roundPerfValue(performance.now() - validationStart),
      });
      ...
      setLoading(false);
      setLoadingStatus("");
      markLoadStage("first_renderer_ready", {
        stageMs: roundPerfValue(performance.now() - rendererStart),
        usedFallback,
      });
      onModelFirstFrameReady?.();
      const postFirstFrameWork = (async () => {
        await waitForNextPaint();
        await waitForIdleSlice(150);
      })();
      void (async () => {
        await postFirstFrameWork;
        if (!isCurrentLoad()) {
          markLoadStage("store_model_commit_skipped_stale");
          return;
        }
        const storeCommitStart = performance.now();
        ignoreNextModelDataUpdate.current = true;
        onModelLoaded(model);
        markLoadStage("store_model_committed", {
          stageMs: roundPerfValue(performance.now() - storeCommitStart),
        });
      })().catch((storeError) => {
        markLoadStage("store_model_commit_error", {
      ...
      void (async () => {
        await postFirstFrameWork;
        await waitForNextPaint();
        await waitForIdleSlice(250);
        const textureStart = performance.now();
        markLoadStage("texture_background_start", {
          textureCount: textureLoadContext.effectiveTexturePaths.length,
        });
        // 模型贴图与队伍色替换贴图互不依赖，并行缩短等待时间；renderer 先显示，贴图后台补齐。
        const [textureResults] = await Promise.all([
          loadAllTextures(model, newRenderer, path, textureWorkers, 512, {
            yieldUploads: true,
            uploadYieldBatch: 2,
            uploadFrameBudgetMs: 6,
            textureLoadContext,
            batchPayloadPromise,
          }),
        ...
        markLoadStage("texture_background_complete", {
          stale: !isCurrentLoad(),
          loaded: textureResults.filter((r) => r.loaded).length,
          missing: textureResults.filter((r) => !r.loaded).length,
          stageMs: roundPerfValue(performance.now() - textureStart),
        });
      })().catch((textureError) => {
        console.error("[Viewer] Background texture loading failed:", textureError);
        markLoadStage("texture_background_error", {
          message: textureError instanceof Error ? textureError.message : String(textureError),
        });
      });
    } catch (error) {
      ...
      setLoading(false);setLoadingStatus("");reportModelLoadError(error, path);
    }
  };
  ...
  const reloadRendererWithData = async (model: any, modelPath: string) => {
    ...
    try {
      ...
      const modelForReload = (!Array.isArray(model?.Materials) || model.Materials.length === 0) && Array.isArray(renderer?.model?.Materials) && renderer.model.Materials.length > 0 ? { ...model, Materials: renderer.model.Materials } : model;
  ...
  const runQueuedTextureReload = useCallback(
    (request: TextureReloadRequest) => {
      const scheduler = textureReloadSchedulerRef.current;
      scheduler.running = true;

      const shouldUseResult = () => scheduler.version === request.version && request.renderer === rendererRef.current;
      ...
      void loadAllTextures(request.renderer.model, guardedRenderer, request.modelPath, textureWorkers, undefined, {
        yieldUploads: true,
  ...
  const scheduleTextureReload = useCallback(
    (request: Omit<TextureReloadRequest, "version">) => {
      const scheduler = textureReloadSchedulerRef.current;
      scheduler.version += 1;
      scheduler.queued = {
        ...request,
  ...
  useEffect(() => {
    ...
    if (ignoreNextModelDataUpdate.current && modelDataChanged && !hasChanged && renderer && modelData) {
      ...
      if (Array.isArray(modelData.Geosets) && Array.isArray(renderer.model?.Geosets)) {
        const minLen = Math.min(modelData.Geosets.length, renderer.model.Geosets.length);
        for (let i = 0; i < minLen; i++) {
          const sourceGeoset = modelData.Geosets[i];
          const rendererGeoset = renderer.model.Geosets[i] as any;
          if (!sourceGeoset || !rendererGeoset) continue;
      ...
      markStandalonePerf("viewer_modeldata_sync_skipped", {
        modelPath: modelPath || (modelData as any)?.__modelPath || "",
        reason: "initial_store_commit",
      });
      return;
    }
    ...
    if (((!isInitialMount && hasChanged) || modelDataChanged) && renderer && modelData) {
      ...
      const expectedModelPath = normalizePath(modelPath || modelPathHint || "").toLowerCase();
      const rendererModelPath = normalizePath((renderer as any).__modelPath || (renderer.model as any)?.__modelPath || (renderer.model as any)?.path || "").toLowerCase();
      if (expectedModelPath && rendererModelPath && expectedModelPath !== rendererModelPath) {
        return;
      }
      ...
      if (modelData.Nodes && modelData.Nodes.length > 0) {
        ...
        rendererSyncService.syncNodeStructure({
          ...
          documentRevision: useModelStore.getState().documentRevision,
          previewRevision: useModelStore.getState().previewRevision,
          ensureNodes: (rendererModel) => ensureRenderNodes(rendererModel as any).model.Nodes,
        });
      }
      ...
      if (modelData.ParticleEmitters2 && modelData.ParticleEmitters2.length > 0) {
        const texturesForValidation = modelData.Textures && modelData.Textures.length > 0 ? modelData.Textures : renderer.model?.Textures && renderer.model.Textures.length > 0 ? renderer.model.Textures : modelData.Textures || [];
      ...
      rendererSyncService.syncSceneMetadata({
        renderer: renderer as any,
        document: modelData as any,
        particleEmitters: nextEmitters as any,
        documentRevision: useModelStore.getState().documentRevision,
        previewRevision: useModelStore.getState().previewRevision,
        syncParticleEmittersInPlace: (currentEmitters, emittersForSync) =>
          syncParticleEmitters2InPlace(currentEmitters as any[], emittersForSync as any[]),
      });
      if (import.meta.env.DEV) {
      }
      ...
      if (Array.isArray(modelData.Materials) && modelData.Materials.length > 0) {
        const revisionState = useModelStore.getState();
        const materialSyncResult = rendererSyncService.syncDocumentMaterials({
          renderer: renderer as any,
          document: modelData as any,
          documentRevision: revisionState.documentRevision,
          previewRevision: revisionState.previewRevision,
          prepareMaterialsForRenderer: (materials, texturesLength) =>
            cloneMaterialsWithReferenceBlendCompat(sanitizeMaterialsForRenderer(materials as any[] | undefined, texturesLength)),
        });
        if (materialSyncResult.plan === "fullReload") {
          useModelStore.getState().triggerRendererReload();
        }
      ...
      rendererSyncService.syncGeosetBuffers({
        renderer: renderer as any,
        document: modelData as any,
        documentRevision: useModelStore.getState().documentRevision,
        previewRevision: useModelStore.getState().previewRevision,
      ...
      rendererSyncService.syncGeosetMaterialBindings({
        renderer: renderer as any,
        document: modelData as any,
        documentRevision: useModelStore.getState().documentRevision,
        previewRevision: useModelStore.getState().previewRevision,
      ...
      rendererSyncService.syncAnimationMetadata({
        renderer: renderer as any,
        document: modelData as any,
        documentRevision: useModelStore.getState().documentRevision,
        previewRevision: useModelStore.getState().previewRevision,
        ensureSequences: (rendererModel) => ensureRendererSequences(rendererModel as any).model.Sequences,
      });
  ...
  useEffect(() => {
    ...
    if (renderer && canvasRef.current) {
      const gl = glRef.current || (canvasRef.current.getContext("webgl2", WEBGL_CONTEXT_ATTRIBUTES) as WebGL2RenderingContext | null) || (canvasRef.current.getContext("webgl", WEBGL_CONTEXT_ATTRIBUTES) as WebGLRenderingContext | null);
      ...
      const render = (time: DOMHighResTimeStamp, scheduleNext = true) => {
        ...
        try {
          ...
          if (isPlayingRef.current && shouldAdvanceGlobalSequencesInResetPose && mdlRenderer.rendererData) {
            ...
            const activeGlobalSequenceDuration = isSpecificGlobalSequenceView ? Number(globalSequenceDurations?.[activeGlobalSequenceFilter] ?? 0) : null;
            const advanceDelta = delta * playbackSpeedRef.current;

            if (!isLoopingRef.current && mdlRenderer.rendererData) {
              if (isSpecificGlobalSequenceView && activeGlobalSequenceDuration !== null && Array.isArray(mdlRenderer.rendererData.globalSequencesFrames)) {
            ...
            if (!isLoopingRef.current && mdlRenderer.rendererData && ((isSpecificGlobalSequenceView && activeGlobalSequenceDuration !== null && Array.isArray(mdlRenderer.rendererData.globalSequencesFrames) && mdlRenderer.rendererData.globalSequencesFrames[activeGlobalSequenceFilter] >= activeGlobalSequenceDuration - 0.1) || (!isSpecificGlobalSequenceView && mdlRenderer.rendererData.animationInfo && mdlRenderer.rendererData.frame >= mdlRenderer.rendererData.animationInfo.Interval[1] - 0.1))) {
              onTogglePlay();
            }
            ...
            if (currentMainMode === "animation" && mdlRenderer.rendererData) {
              ...
              if (time - lastFrameSyncTime.current >= FRAME_SYNC_INTERVAL) {
                const timelineFrame = isSpecificGlobalSequenceView && activeGlobalSequenceDuration !== null && Array.isArray(mdlRenderer.rendererData.globalSequencesFrames) ? Number(mdlRenderer.rendererData.globalSequencesFrames[activeGlobalSequenceFilter] ?? 0) : mdlRenderer.rendererData.frame;
            ...
            if (mdlRenderer.rendererData) {
              if (currentMainMode === "animation") {
                if (Math.abs(storeFrame - currentFrame) > 0.0001) {
                  const activeGlobalSequenceFilter = useSelectionStore.getState().timelineGlobalSequenceFilter;
                  const globalSequenceDurations = mdlRenderer.model?.GlobalSequences;
                  const activeGlobalSequenceDuration = typeof activeGlobalSequenceFilter === "number" && activeGlobalSequenceFilter >= 0 ? Number(globalSequenceDurations?.[activeGlobalSequenceFilter] ?? NaN) : NaN;
                  const hasSpecificGlobalSequence = Number.isFinite(activeGlobalSequenceDuration) && activeGlobalSequenceDuration >= 0;

                  const interval = mdlRenderer.rendererData.animationInfo?.Interval;
                  const hasInterval = !!interval && typeof (interval as any).length === "number" && (interval as any).length >= 2;

                  if ((hasSpecificGlobalSequence || hasInterval) && Number.isFinite(storeFrame)) {
                    const safeInterval = interval ?? [0, 0];
                    const targetFrame = hasSpecificGlobalSequence ? Math.min(Math.max(storeFrame, 0), activeGlobalSequenceDuration) : Math.min(Math.max(storeFrame, Number(safeInterval[0])), Number(safeInterval[1]));

                    // Deterministic paused preview:
                    // Keep a single authoritative frame for ribbons and other anim channels.
                    const globalSequenceDurations = mdlRenderer.model?.GlobalSequences;
                    const globalSequenceCount = typeof (globalSequenceDurations as ArrayLike<number> | undefined)?.length === "number" ? Number((globalSequenceDurations as ArrayLike<number>).length) : 0;
          ...
          if (gl && showCamerasRef.current) {
            ...
            if (cameraNodes.length > 0 && selectedIdx >= 0 && selectedIdx < cameraNodes.length) {
              ...
              debugRenderer.current.renderWireframeFrustum(gl, mvMatrix, pMatrix, pos, target, fov, nearClip, farClip, [0, 0.8, 1, 1]);
            }
          }
          ...
          if (mdlRenderer.rendererData) {
            ...
            const visibleRendererNodes = filterVisibleRendererNodes(mdlRenderer.rendererData.nodes, hiddenNodeIdSet, useRendererStore.getState().nodeTypeVisibility);
            ...
            if (envManager.isEnabled()) {
              ...
              if ((mdlRenderer as any).setEnvironmentLight) {
                (mdlRenderer as any).setEnvironmentLight(envParams.lightDirection, envParams.lightColor, envParams.ambientColor);
              }
            }
            ...
            const shouldRenderWireframeOverlay = showWireframeOverlayRef.current && !showWireframeRef.current;
            ...
            try {
              ...
              if (shouldRenderWireframeOverlay) {
                ...
                mdlRenderer.render(mvMatrix, pMatrix, {
                  ...renderOpts,
                  wireframe: true,
              ...
              if (healthBarState) {
                ...
                if (billboard) {
                  ...
                  const buildQuad = (halfWidth: number, halfThickness: number) => {
                    ...
                    return {
                      triangles: [topLeft[0], topLeft[1], topLeft[2], bottomLeft[0], bottomLeft[1], bottomLeft[2], topRight[0], topRight[1], topRight[2], topRight[0], topRight[1], topRight[2], bottomLeft[0], bottomLeft[1], bottomLeft[2], bottomRight[0], bottomRight[1], bottomRight[2]],
                      outline: [topLeft[0], topLeft[1], topLeft[2], topRight[0], topRight[1], topRight[2], topRight[0], topRight[1], topRight[2], bottomRight[0], bottomRight[1], bottomRight[2], bottomRight[0], bottomRight[1], bottomRight[2], bottomLeft[0], bottomLeft[1], bottomLeft[2], bottomLeft[0], bottomLeft[1], bottomLeft[2], topLeft[0], topLeft[1], topLeft[2]],
                    };
                  };
                  ...
                  debugRenderer.current.renderTriangles(gl as WebGLRenderingContext, mvMatrix, pMatrix, outerQuad.triangles, [0, 0, 0, 1]);
                  debugRenderer.current.renderTriangles(gl as WebGLRenderingContext, mvMatrix, pMatrix, innerQuad.triangles, [0.12, 0.78, 0.18, 1]);
                  debugRenderer.current.renderLines(gl as WebGLRenderingContext, mvMatrix, pMatrix, outerQuad.outline, outlineColor);
                }
              }
            }
              ...
              if (geoset && geoset.Faces && geoset.Vertices) {
                ...
                if (isWireframeMode && gl && debugRenderer.current) {
                  ...
                  for (let i = 0; i < faces.length; i += 3) {
                    ...
                    positions.push(skinnedVerts[i1], skinnedVerts[i1 + 1], skinnedVerts[i1 + 2], skinnedVerts[i2], skinnedVerts[i2 + 1], skinnedVerts[i2 + 2], skinnedVerts[i3], skinnedVerts[i3 + 1], skinnedVerts[i3 + 2]);
                  }
                  gl.disable(gl.DEPTH_TEST);
                  debugRenderer.current.renderTriangles(gl as WebGLRenderingContext, mvMatrix, pMatrix, positions, [hoverColorRgb[0], hoverColorRgb[1], hoverColorRgb[2], 1], false);
                  gl.enable(gl.DEPTH_TEST);
                }
              }
            }
                ...
                if (selectedNode) {
                  ...
                  if (currentMainMode === "animation" && currentAnimationSubMode === "binding" && debugRenderer.current) {
                    ...
                    if (boundVertices.length > 0 && showVerticesRef.current) {
                      ...
                      debugRenderer.current.renderBoneVertices(gl as WebGLRenderingContext, mvMatrix, pMatrix, boundVertices);
                      gl.enable(gl.DEPTH_TEST);
                    }
                  }
                }
                ...
                try {
                  ...
                  if (selectedIdNums.length === 0) {
                    ...
                    const resolveScalarAtFrame = (prop: any, frame: number, fallback: number): number => {
                      ...
                      const keys = keysRaw.filter((k: any) => k && !k._isPreviewKey && Number.isFinite(Number(k.Frame))).sort((a: any, b: any) => Number(a.Frame) - Number(b.Frame));
                    ...
                    for (const nodeId of selectedIdNums) {
                      const nodeWrapper = rdNodes[nodeId] ?? (mdlRenderer.rendererData.nodes as any[]).find((n: any) => Number(n?.node?.ObjectId) === nodeId);
                      ...
                      const widthProp = pe2Node?.Width ?? pe2Node?.WidthAnim ?? pe2Node?.props?.Width ?? nodeData?.Width ?? nodeData?.WidthAnim ?? storeNode?.Width ?? storeNode?.WidthAnim;
                      const lengthProp = pe2Node?.Length ?? pe2Node?.LengthAnim ?? pe2Node?.props?.Length ?? nodeData?.Length ?? nodeData?.LengthAnim ?? storeNode?.Length ?? storeNode?.LengthAnim;
                      ...
                      const pivot = nodeData?.PivotPoint || pe2Node?.PivotPoint || storeNode?.PivotPoint || [0, 0, 0];
                      ...
                      if (dirLen > 1e-6) {
                        ...
                        pushLine([pivotWorld[0], pivotWorld[1], pivotWorld[2]], [tipPt[0], tipPt[1], tipPt[2]]);
                        ...
                        for (const [sx, sy] of headPts) {
                          ...
                          pushLine([tipPt[0], tipPt[1], tipPt[2]], [cornerPt[0], cornerPt[1], cornerPt[2]]);
                        }
                      }
                    }

                    if (linePositions.length > 0) {
                      debugRenderer.current.renderLines(gl as WebGLRenderingContext, mvMatrix, pMatrix, linePositions, [1, 0.35, 0.8, 1]);
                    }
                  }
                } catch (err) {
            ...
            if (gl && showLightsRef.current && mdlRenderer.rendererData.nodes) {
              ...
              if (lightNodes.length > 0) {
                ...
                lightNodes.forEach((light: any) => {
                  ...
                  debugRenderer.current.renderLight(gl as WebGLRenderingContext, nodeMVMatrix, projectionMatrix, type, attStart, attEnd, color);
                });
              }
            }
              ...
              if (collisionNodes.length > 0) {
                ...
                collisionNodes.forEach((nodeWrapper: any) => {
                  ...
                  if (isSphere) {
                    ...
                    if (v1 && v2 && gl) {
                      debugRenderer.current.renderWireframeBox(gl as WebGLRenderingContext, nodeMVMatrix, projectionMatrix, v1, v2, [1, 0.5, 0, 1]);
                    }
                  }
                });
              }
          ...
          if (gl && (currentMainMode === "view" || currentMainMode === "uv" || (currentMainMode === "geometry" && geometrySubMode === "vertex") || (currentMainMode === "animation" && currentAnimationSubMode === "binding"))) {
            ...
            const { uvSelectionMode } = useSelectionStore.getState();
            const showUvViewerSelectionHighlight = useUvEditorStore.getState().showViewerSelectionHighlight;
            const shouldRenderUvSelection = currentMainMode !== "uv" || showUvViewerSelectionHighlight;
            ...
            const hoverColorRgb = hexToRgb(hoverColor);
            const shouldRenderSelectedVertices = shouldRenderUvSelection && selectedVertexIds.length > 0 && (showVerticesRef.current || currentMainMode === "uv");
            const shouldRenderGroupEdges = (currentMainMode === "geometry" && geometrySubMode === "group") || (currentMainMode === "uv" && shouldRenderUvSelection && uvSelectionMode === "group");
            const shouldRenderUvEdges = currentMainMode === "uv" && shouldRenderUvSelection && uvSelectionMode === "edge";
            ...
            if (showVerticesRef.current) {
              for (let geosetIndex = 0; geosetIndex < mdlRenderer.model.Geosets.length; geosetIndex++) {
                ...
                if (!isCurrentGeosetVisible(geosetIndex)) continue;
                ...
                if (hoveredGeosetId === geosetIndex) {
                  ...
                  if (canUseCachedVertexBuffer) {
                    debugRenderer.current.renderCachedPoints(gl as WebGLRenderingContext, mvMatrix, pMatrix, geoset.Vertices, [...hoverColorRgb, 1], hoverPointSize, vertexSettingsRef.current.enableDepth, true, effectiveVertexRenderRevision);
                  } else {
                  ...
                  if (canUseCachedVertexBuffer) {
                    debugRenderer.current.renderCachedPoints(gl as WebGLRenderingContext, mvMatrix, pMatrix, geoset.Vertices, [...vertexColorRgb, 0.8], basePointSize, vertexSettingsRef.current.enableDepth, true, effectiveVertexRenderRevision);
                  } else {
            ...
            if (shouldRenderSelectedVertices) {
              const selectedPositions: number[] = [];

              if (shouldRenderGroupEdges) {
                const linePositions: number[] = [];
                for (const sel of selectedFaceIds) {
                  if (!isCurrentGeosetVisible(sel.geosetIndex)) continue;
                ...
                if (shouldRenderUvEdges) {
                  const selectedVertexKeySet = new Set(selectedVertexIds.map((sel) => `${sel.geosetIndex}:${sel.index}`));
                  ...
                  for (let geosetIndex = 0; geosetIndex < mdlRenderer.model.Geosets.length; geosetIndex++) {
                    if (!isCurrentGeosetVisible(geosetIndex)) continue;
                    const geoset = mdlRenderer.model.Geosets[geosetIndex];
                    if (!geoset?.Faces || !geoset?.Vertices) {
                      continue;
                    }
                    ...
                    for (let faceOffset = 0; faceOffset < geoset.Faces.length; faceOffset += 3) {
                      ...
                      for (const [startIndex, endIndex] of edges) {
                        if (!selectedVertexKeySet.has(`${geosetIndex}:${startIndex}`) || !selectedVertexKeySet.has(`${geosetIndex}:${endIndex}`)) {
                          continue;
                        }

                        const edgeKey = startIndex < endIndex
                          ? `${geosetIndex}:${startIndex}-${endIndex}`
                          : `${geosetIndex}:${endIndex}-${startIndex}`;
                        ...
                        linePositions.push(
                          geoset.Vertices[startOffset], geoset.Vertices[startOffset + 1], geoset.Vertices[startOffset + 2],
                          geoset.Vertices[endOffset], geoset.Vertices[endOffset + 1], geoset.Vertices[endOffset + 2]
                        );
                      }
                    }
                  }
                ...
                for (const sel of selectedVertexIds) {
                  ...
                  if (geoset) {
                    ...
                    selectedPositions.push(geoset.Vertices[vIndex], geoset.Vertices[vIndex + 1], geoset.Vertices[vIndex + 2]);
                  }
                }
                gl.disable(gl.DEPTH_TEST);
          ...
          if (gl && ((currentMainMode === "geometry" && (geometrySubMode === "face" || geometrySubMode === "group")) || (currentMainMode === "uv" && useUvEditorStore.getState().showViewerSelectionHighlight && (useSelectionStore.getState().uvSelectionMode === "face" || useSelectionStore.getState().uvSelectionMode === "group" || useSelectionStore.getState().uvSelectionMode === "block")))) {
            if (selectedFaceIds.length > 0) {
              ...
              for (const sel of selectedFaceIds) {
                ...
                if (geoset) {
                  ...
                  selectedPositions.push(geoset.Vertices[i1], geoset.Vertices[i1 + 1], geoset.Vertices[i1 + 2], geoset.Vertices[i2], geoset.Vertices[i2 + 1], geoset.Vertices[i2 + 2], geoset.Vertices[i3], geoset.Vertices[i3 + 1], geoset.Vertices[i3 + 2]);
                }
              }
              gl.disable(gl.DEPTH_TEST);
              ...
              debugRenderer.current.renderLines(gl as WebGLRenderingContext, mvMatrix, pMatrix, linePositions, [...selectionColorRgb, 1], false);
              gl.enable(gl.DEPTH_TEST);
            }
          }
          ...
          if (transformMode) {
            ...
            if (isGlobalTransformMode) {
              ...
              count = 1;
              showGizmo = true;
            } else if (currentMainMode === "geometry") {
              if ((geometrySubMode === "vertex" || geometrySubMode === "group") && selectedVertexIds.length > 0) {
                ...
                for (const sel of selectedFaceIds) {
                  ...
                  if (geoset) {
                    const fIndex = sel.index * 3;
                    const i1 = geoset.Faces[fIndex] * 3,
                      i2 = geoset.Faces[fIndex + 1] * 3,
                      i3 = geoset.Faces[fIndex + 2] * 3;
  ...
  const getModelCenter = (): [number, number, number] => {
    ...
    const getGeosetBoundsCenter = (geosets: any): [number, number, number] | null => {
      ...
      return count > 0 ? centerFromBounds(minX, minY, minZ, maxX, maxY, maxZ) : null;
    };

    const toStaticVec3 = (value: any, fallback: [number, number, number]): [number, number, number] => {
      const source = value?.Keys?.[0]?.Vector ?? value;
    ...
    const toStaticQuat = (value: any): quat => {
      const source = value?.Keys?.[0]?.Vector ?? value;
      const x = Number(source?.[0]);
      const y = Number(source?.[1]);
      const z = Number(source?.[2]);
    ...
    const getCommittedGlobalTransformMatrix = (): mat4 | null => {
      const nodes = Array.isArray(modelState.nodes) && modelState.nodes.length > 0
        ? modelState.nodes
        : (Array.isArray(modelData?.Nodes) ? modelData.Nodes : []);
      const transformNodes = nodes.filter((node: any) => {
        const name = String(node?.Name ?? "");
        return name === "__WMV_GLOBAL_TRANSFORM_ROOT__" || name.startsWith("__WMV_MIRROR_ROOT__");
      });
      if (transformNodes.length === 0) return null;

      const transformIds = new Set(transformNodes.map((node: any) => Number(node?.ObjectId)).filter(Number.isFinite));
      const byId = new Map<number, any>();
      for (const node of transformNodes) {
        const id = Number((node as any)?.ObjectId);
        if (Number.isFinite(id)) byId.set(id, node);
      }

      const innermost = transformNodes.find((node: any) => {
        const id = Number(node?.ObjectId);
        return Number.isFinite(id) && !transformNodes.some((candidate: any) => Number(candidate?.Parent) === id);
      }) ?? transformNodes[0];

      const chain: any[] = [];
      let current: any | undefined = innermost;
      const visited = new Set<number>();
      while (current) {
        const id = Number(current.ObjectId);
        if (!Number.isFinite(id) || visited.has(id)) break;
        visited.add(id);
        chain.unshift(current);

        const parentId = Number(current.Parent);
        current = transformIds.has(parentId) ? byId.get(parentId) : undefined;
      }

      const composite = mat4.create();
      for (const node of chain) {
        const translation = toStaticVec3(node.Translation, [0, 0, 0]);
        const rotation = toStaticQuat(node.Rotation);
        const scaling = toStaticVec3(node.Scaling, [1, 1, 1]);
        const pivot = toStaticVec3(node.PivotPoint, [0, 0, 0]);
        const local = mat4.create();
        mat4.fromRotationTranslationScaleOrigin(local, rotation, translation, scaling, pivot);
        mat4.multiply(composite, composite, local);
      }

      return composite;
    };

    const applyCommittedGlobalTransform = (center: [number, number, number]): [number, number, number] => {
      const matrix = getCommittedGlobalTransformMatrix();
      if (!matrix) return center;

      const transformed = vec3.fromValues(center[0], center[1], center[2]);
      vec3.transformMat4(transformed, transformed, matrix);
  ...
  const sanitizeTransformValue = (propertyName: "Translation" | "Rotation" | "Scaling", value: any, defaultVal: number[]): number[] => {
  ...
  const handleMouseMove = (e: any) => {
    if (canvasRef.current) {const rect=canvasRef.current.getBoundingClientRect();hoveredNodeNameIdRef.current=findClosestNodeLabel(nodeNameLabelsRef.current,(e.clientX-rect.left)*canvasRef.current.width/Math.max(1,rect.width),(e.clientY-rect.top)*canvasRef.current.height/Math.max(1,rect.height),20)?.id??null;}
    // 1. Gizmo Dragging
    ...
    if (gizmoState.current.isDragging && gizmoState.current.activeAxis && rendererRef.current) {
      ...
      if (!isGlobalTransformMode && !isHealthBarDrag && mainMode !== "geometry" && !(mainMode === "animation" && (subMode === "binding" || subMode === "keyframe"))) {
        return;
      }
      ...
      const gizmoOrientation = useRendererStore.getState().gizmoOrientation;
      const axisBasis = gizmoOrientation === "camera" ? { x: vecs.right, y: vecs.camUp, z: vecs.forward } : { x: vec3.fromValues(1, 0, 0), y: vec3.fromValues(0, 1, 0), z: vec3.fromValues(0, 0, 1) };
      ...
      const viewSize = cameraRef.current?.projectionMode === "orthographic" ? cameraRef.current.orthoSize : distance;
      ...
      const getRay = (sx: number, sy: number) => {
        ...
        const invProj = mat4.create();
        mat4.invert(invProj, viewMatrices.pMatrix);
        const invView = mat4.create();
        mat4.invert(invView, viewMatrices.mvMatrix);
        ...
        vec3.subtract(dir, target, origin);
      ...
      if (isSingleAxis && gizmoCount > 0) {
        ...
        if (viewMatrices && hasCanvas) {
          ...
          vec3.add(axisEnd, gizmoCenter, axisDir);
      ...
      if (isGlobalTransformMode) {
        ...
        if (transformMode === "translate") {
          ...
          applyTranslateSnap(vecs.moveVec);
          applyHudTranslate(vecs.moveVec);
          previewTransform.translation = [previewTransform.translation[0] + vecs.moveVec[0], previewTransform.translation[1] + vecs.moveVec[1], previewTransform.translation[2] + vecs.moveVec[2]];
      ...
      if (transformMode === "translate" && mainMode === "geometry") {
        ...
        applyHudTranslate(vecs.moveVec);
        const { selectedVertexIds, selectedFaceIds, geometrySubMode } = useSelectionStore.getState();
        const expandedFaceSelection = geometrySubMode === "face" ? getExpandedFaceVertexSelection(selectedFaceIds as Array<{ geosetIndex: number; index: number }>) : [];
        ...
        const effectiveNodeIds = subMode === "keyframe" ? selectedNodeIds.filter((id) => !hasSelectedAncestor(id)) : selectedNodeIds;
            ...
            if (nodeWrapper && pivot) {
              ...
              if (subMode === "keyframe") {
                ...
                if (parentId !== undefined && parentId >= 0) {
                  ...
                  const parentWrapper = rendererRef.current!.rendererData.nodes.find((n: any) => n.node.ObjectId === parentId);
                  ...
                  if (parentWrapper?.matrix) {
                    ...
                    const invParentRot = mat3.fromValues(
                      pm[0],
                      pm[4],
                      pm[8], // Transpose row 0 = column 0
                      pm[1],
                      pm[5],
                      pm[9], // Transpose row 1 = column 1
                      pm[2],
                      pm[6],
                      pm[10], // Transpose row 2 = column 2
                    );
              ...
              // keyframe 模式：累计 delta 并注入临时 Translation 关键帧用于实时预览
                ...
                if (dragData && rendererRef.current) {
                  ...
                  if (rendererNode) {
                    ...
                    const existingKey = translationKeys.find((k: any) => !k._isPreviewKey && Math.abs(k.Frame - frame) < 0.1);
                    ...
                    const previewTranslation = [baseTranslation[0] + delta[0], baseTranslation[1] + delta[1], baseTranslation[2] + delta[2]];
        ...
        if (count > 0) {
          ...
          if (transformMode === "rotate") {
            ...
            if (axis === "x") scaleVec[0] = scaleFactor;
              ...
              vec3.set(scaleVec, scaleFactor, scaleFactor, scaleFactor);
            }
            ...
            selectedNodeIds.forEach((nodeId) => {
              ...
              if (currentVal) {
                ...
                if (parentId !== undefined && parentId !== null && parentId >= 0) {
                  const parentWrapper = rendererRef.current!.rendererData.nodes.find((n: any) => n && n.node && n.node.ObjectId === parentId);
                  if (parentWrapper?.matrix) {
  ...
  const handleAutoSeparateLayers = () => {
    if (!rendererRef.current) {
      appMessage.warning("当前渲染器未就绪");
      return;
    }
    const { mainMode } = useSelectionStore.getState();
    if (mainMode !== "geometry") {
      appMessage.warning("请先切换到顶点模式");
      return;
    }

    appMessage.loading({ content: "正在执行一键分层...", key: "auto-separate-layers", duration: 0 });
    const cmd = new AutoSeparateLayersCommand(rendererRef.current);
    commandManager.execute(cmd);

    if (!cmd.lastResult) {
      appMessage.warning({ content: "一键分层未返回结果", key: "auto-separate-layers" });
      return;
    }

    if (cmd.lastResult.changedGeosetCount <= 0) {
      appMessage.info({ content: "当前模型不需要分层", key: "auto-separate-layers" });
      return;
    }
  ...
  const handleSeparateConfirm = (config: { mode: "keep" | "new" | "existing"; materialIndex?: number; newLayerConfig?: LayerConfig }) => {
    if (!rendererRef.current) return;
    const splitSelection = getSplitVertexSelection();
    ...
    if (wasGizmoDragging) {
      ...
      if (rendererRef.current) {
        ...
        if (isHealthBarSelectedRef.current && healthBarDragInitialRef.current) {
          ...
          if (hasMoved) {
            const newMinimumExtent: [number, number, number] = [start.minimumExtent[0] + offset[0], start.minimumExtent[1] + offset[1], start.minimumExtent[2] + offset[2]];
            const newMaximumExtent: [number, number, number] = [start.maximumExtent[0] + offset[0], start.maximumExtent[1] + offset[1], start.maximumExtent[2] + offset[2]];
          ...
          if (initialVertexPositions.current.size > 0) {
            ...
            if (changes.length > 0) {
              const cmd = new MoveVerticesCommand(rendererRef.current, changes, (syncedChanges) => {
          ...
          if (initialNodePositions.current.size > 0) {
            ...
            if (changes.length > 0) {
              const cmd = new MoveNodesCommand(rendererRef.current, changes, (syncedChanges) => {
              ...
              selectedNodeIds.forEach((nodeId) => {
                ...
                if (delta && (delta[0] !== 0 || delta[1] !== 0 || delta[2] !== 0)) {
                  ...
                  if (storeNode) {
                    ...
                    const newTranslation = [baseTranslation[0] + delta[0], baseTranslation[1] + delta[1], baseTranslation[2] + delta[2]];
            ...
            if (autoKeyframe) {
              ...
              selectedNodeIds.forEach((nodeId) => {
                ...
                if (transformMode === "rotate") {
                  ...
                  const existingKey = storeNode.Rotation?.Keys?.find((k: any) => Math.abs(k.Frame - frame) < 0.1);
                  const oldValue = existingKey?.Vector ? (Array.isArray(existingKey.Vector) ? [...existingKey.Vector] : Array.from(existingKey.Vector)) : null;
                  const baseValue = existingKey?.Vector ? (Array.isArray(existingKey.Vector) ? [...existingKey.Vector] : Array.from(existingKey.Vector)) : interpolateValue(storeNode.Rotation?.Keys, frame, [0, 0, 0, 1]);
                  if (!oldValue && isSameVec(newValue, baseValue)) return;
                  if (oldValue && isSameVec(newValue, oldValue)) return;
                  changes.push({
                    nodeId,
                  ...
                  const existingKey = storeNode.Scaling?.Keys?.find((k: any) => Math.abs(k.Frame - frame) < 0.1);
                  const oldValue = existingKey?.Vector ? (Array.isArray(existingKey.Vector) ? [...existingKey.Vector] : Array.from(existingKey.Vector)) : null;
                  const baseValue = existingKey?.Vector ? (Array.isArray(existingKey.Vector) ? [...existingKey.Vector] : Array.from(existingKey.Vector)) : interpolateValue(storeNode.Scaling?.Keys, frame, [1, 1, 1]);
                  if (!oldValue && isSameVec(newValue, baseValue)) return;
                  if (oldValue && isSameVec(newValue, oldValue)) return;
                  changes.push({
                    nodeId,
  ...
  const handleDeleteVertices = () => {
    ...
    if (mainMode !== "geometry") return false;

    if (geometrySubMode === "vertex") {
      if (selectedVertexIds.length < 1) return false;
    ...
    if (geometrySubMode === "face" || geometrySubMode === "group") {
      if (selectedFaceIds.length > 0) {
        const cmd = new DeleteFacesCommand(rendererRef.current, selectedFaceIds);
        commandManager.execute(cmd);
        return true;
      }

      if (geometrySubMode === "group" && selectedVertexIds.length > 0) {
        const cmd = new DeleteVerticesCommand(rendererRef.current, selectedVertexIds);
        commandManager.execute(cmd);
        return true;
      }
  ...
  const handlePasteVertices = () => {
    ...
    const cmd = new PasteVerticesCommand(rendererRef.current, vertexCopyBuffer.current, pasteCreatesNewGeoset, [10, 0, 0], vertexCopySelectionModeRef.current ?? selectionMode);
    commandManager.execute(cmd);
  };
  ...
  const pastePoseAtCurrentFrame = (): boolean => {
    ...
    if (!nodes || nodes.length === 0) return false;
    const frame = Math.round(currentFrame);
    const nodeMap = new Map(nodes.filter((n: any) => typeof n.ObjectId === "number" && n.ObjectId >= 0 && n.type !== NodeType.CAMERA).map((n: any) => [n.ObjectId, n]));
    const changes: KeyframeChange[] = [];

    const pushChange = (nodeId: number, propertyName: "Translation" | "Rotation" | "Scaling", value: number[], defaultVal: number[]) => {
  ...
  useEffect(() => {
    const hasGeometryDeleteSelection = () => {
      const { mainMode, geometrySubMode, selectedVertexIds, selectedFaceIds } = useSelectionStore.getState();
      if (mainMode !== "geometry") return false;
      if (geometrySubMode === "vertex") return selectedVertexIds.length > 0;
      if (geometrySubMode === "face") return selectedFaceIds.length > 0;
      if (geometrySubMode === "group") return selectedFaceIds.length > 0 || selectedVertexIds.length > 0;
      return false;
    };
    ...
    const unsubscribeHandlers = [
      ...
      registerShortcutHandler("geometry.deleteVertices", () => {
        if (!hasGeometryDeleteSelection()) return false;
        return handleDeleteVertices();
      }),
    ];
  ...
  useEffect(() => {
    return registerGeometryDeleteKeyListener((event) => {
      if (event.repeat || isTextInputActive()) {
        return false;
      }
      ...
      const deleteBindings = getEffectiveBindings("geometry.deleteVertices").map(normalizeKeyCombo);
      if (!deleteBindings.includes("Delete")) {
        return false;
      }
      const { mainMode, geometrySubMode, selectedVertexIds, selectedFaceIds } = useSelectionStore.getState();
      const hasSelection =
        geometrySubMode === "vertex"
          ...
          : geometrySubMode === "face"
            ? selectedFaceIds.length > 0
            : geometrySubMode === "group"
              ? selectedFaceIds.length > 0 || selectedVertexIds.length > 0
              : false;
      if (mainMode !== "geometry" || !hasSelection) {
        return false;
      }
      return handleDeleteVertices();
    });
  ...
  return (
    <div style={{ width: "100%", height: "100%", position: "relative", overflow: "hidden" }}>
      <canvas
        ref={canvasRef}
        tabIndex={-1}
        style={{ width: "100%", height: "100%", display: isTexturePreviewMode ? "none" : "block", outline: "none" }}
        ...
        onWheel={handleWheel}
      />
      <canvas ref={nodeNameOverlayRef} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", display: isTexturePreviewMode ? "none" : "block" }} />
      ...
      {!isTexturePreviewMode && appMainMode === "geometry" && (
        <VertexEditor
          renderer={renderer}
          onBeginUpdate={() => {
            ignoreNextModelDataUpdate.current = true;
      ...
      {!isTexturePreviewMode &&
        nodeContextMenu &&
        (() => {
          const node = nodeContextMenu.nodeId !== null ? (stableStoreNodes.find((n) => n.ObjectId === nodeContextMenu.nodeId) ?? null) : null;
          const hasNode = !!node;
      ...
      <GeosetSeparateDialog visible={separateDialogVisible} sourceGeosetIndex={separateSourceGeosetIndex} onCancel={() => setSeparateDialogVisible(false)} onConfirm={handleSeparateConfirm} />
      ...
      {loading && (
        ...
        >
          ...
          <style>{`@keyframes viewer-spin{0%{transform:rotate(0 deg);}100%{transform:rotate(360 deg);}}`}</style>
        </div>
      )}
    </div>
  );
